...
> **Date:** 2026-03-11 (refreshed; submodule refresh v0.0.18 + builder-scaffold a4fb8b0 + evevault v0.0.5 + builder-documentation cf0f3ab)  
...
>
> **BREAKING CHANGES (2026-03-03 submodule refresh — world-contracts v0.0.15, builder-documentation b4178c6):** (1) **Inventory refactored: Item/ItemEntry split.** `ItemEntry` (at-rest, `copy, drop, store`) vs `Item` (in-transit, carries `parent_id` + UID). Deposit validates `parent_id == storage_unit_id` — items only return to origin SSU. (2) **`withdraw_item<Auth>` BREAKING:** Now takes `quantity: u32` + `ctx: &mut TxContext` — supports partial withdrawal. All TradePost call sites must update. (3) **`deposit_item<Auth>` validates `parent_id`** — cross-SSU deposit blocked; use `deposit_to_owned<Auth>` or `transfer::public_transfer` for buyer delivery. (4) **New `deposit_to_owned<Auth>`** — extension pushes items into any player's owned inventory (enables async trading). (5) **AdminACL removed from `deposit_by_owner`/`withdraw_by_owner`** — owner ops now just OwnerCap + sender check. (6) `EItemVolumeMismatch` removed; volume static per type_id. (7) dapp-kit docs simplified: `useSponsoredTransaction` removed, `useDAppKit()` pattern. (8) EVE Vault browser extension docs now populated. (9) Gate/turret/access modules UNCHANGED. **CC extension path (witness-based `withdraw_item<Auth>`, `deposit_item<Auth>`) requires call-site updates but pattern is intact.**
>
> **CHANGES (2026-03-09 submodule refresh — world-contracts v0.0.16/v0.0.17, builder-documentation 1eb5ad4, builder-scaffold 9200be4/v0.0.1, evevault 30f74ef/v0.0.4):** (1) **PlayerProfile struct auto-created** at character creation — transferred to wallet address for client lookup. (2) **Gate type matching enforced:** `link_gates` now requires matching `type_id` (low CC impact). (3) **Turret owner exclusion:** targeting now excludes operator by `character_id` match — Defense Mode turrets won't fire at owner. (4) **Killmail refactored** with `KillmailRegistry` — `create_killmail` signature changed entirely (new params, renamed struct fields); KillmailCreatedEvent field names changed. (5) **Assembly metadata update functions** added to all assembly types + Character (`update_metadata_name/description/url`). (6) **Extension `withdraw_item` online guard** — SSU must be online for extension withdrawal. (7) Gate unlink/unanchor convenience functions added. (8) **builder-documentation major restructure** — Move patterns page, efctl CLI, files reorganized. (9) **builder-scaffold v0.0.1** — PostgreSQL Indexer + GraphQL support. (10) **EVE Vault v0.0.4** — sponsored tx metadata, gas estimation, device reset, EVE token support. **No pattern-breaking changes for CivilizationControl. All validated extension/witness patterns remain intact.**
>
> **CHANGES (2026-03-11 submodule refresh — world-contracts v0.0.18, builder-scaffold a4fb8b0, builder-documentation cf0f3ab, evevault v0.0.5):** (1) **Extension freeze anti-rugpull** — new `extension_freeze.move` module: `freeze_extension_config()` permanently locks authorized extension, `authorize_extension` blocked post-freeze. `ExtensionConfigFrozenEvent` emitted. Shared across gate/turret/SSU. **HIGH CC IMPACT:** Demo trust narrative — freeze gate extension to prove immutable toll rules. (2) **SSU open inventory** — `deposit_to_open_inventory<Auth>` / `withdraw_from_open_inventory<Auth>` for contract-controlled escrow. Extension-only access. `inventory_keys` now starts with 2 entries at anchor. Use cases: prize pools, atomic courier. (3) **Gate/turret new view functions** — `is_extension_configured()`, `extension_type()`, `is_extension_frozen()`. (4) **builder-scaffold docs overhaul** — new `builder-flow-docker.md`, `builder-flow-host.md`, `builder-flow.md`; `sui client test-publish` documented. (5) **EVE Vault v0.0.5** — multi-tenant auth switching (stillness/utopia/testevenet/nebula); wallet standard interface unchanged. **No pattern-breaking changes for CivilizationControl.**
