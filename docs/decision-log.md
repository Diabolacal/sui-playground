...
## 2026-06-27 — Fitting Tool & Overlay Feasibility (research)
- **Goal:** Deep feasibility + product/spec investigation for an EVE Frontier **ship-fitting tool** ("EFT Warrior" for Frontier, built on the new Cycle 6 polyomino fitting UI), plus a secondary deep dive into the EF-Map overlay/helper (technical + visual). Research/spec only — nothing built or deployed.
- **Decision:** Executed as parallel read-only workstreams (3 background subagents over read-only clones of `Diabolacal/EF-Map` @ `b1cd69e` and `Diabolacal/ef-map-overlay` @ `8788a16`; screenshot/engine/UX/CV done in-thread). Produced 16 artifacts under `docs/current/research/frontier-fitting-overlay-feasibility-2026-06-27/`. **Verdict: build the fitting tool** as a **standalone web app** (`fit.ef-map.com`) that reuses EF-Map's public data, short-link service, and industry engine; treat the overlay refresh as a valuable parallel moat, never a blocker. **Recommended next build: Frontier Fitting MVP ("RootFit"),** starting with a ~2–4h data-extraction spike.
- **Key findings:** EF-Map's public `blueprint_data_v5.json` already carries all **16 Cycle 6 modules** (typeIDs 95302–95503) with name/mass/volume/category + build recipes → palette + mass + shopping-list are nearly free. Classic per-type stats (powergrid, HP, velocity, capacitor, warp) are **extractable** via EF-Map's existing `tools/game-data-extractor/extract_ships.py` dogma pipeline (one run). The hard gap is **module footprint shapes + interior/exterior + hardpoint metadata** — **not in any extractable data (~90% confidence)** → hand-author (~16–30 entries). Engine (polyomino placement/power/additive stats) is low-risk and fully specifiable; non-additive stat formulas (velocity/inertia/warp) ship "estimated," calibrated later. Overlay is a generic JSON bridge (HTTP :38765 / WS :38766 / `ef-overlay://`, schema v4) with **no capability negotiation** and its text-card renderer removed → propose schema-v5 `overlay_cards[]` + `capability_hello` + a `fit_plan` card (in-cockpit checklist); visually it still uses ImGui default + ProggyClean → ~1-day Phase-1 refresh (fonts/flat style/DPI/alpha).
- **Operator corrections honored (supersede the prior ideation pass):** Rift Watch **out** (events not emitting; coords likely to be obfuscated when they do); Patch Witness/Frontier Changelog, SSU inventory dashboard, Frontier Facts/MCP, Discord bot **deprioritized**; encrypted tribe packs **excluded**; blueprint permalink only if it serves fitting/overlay. Helper C++ changes + Microsoft Store resubmission are **acceptable** (existing build stays live if review fails).
- **Files:** new `docs/current/research/frontier-fitting-overlay-feasibility-2026-06-27/**` (README, operator-context, data-source-audit, fitting-model-and-rules, product-ux-spec, screenshot-import-feasibility, ef-map-integration-plan, overlay-helper-audit, overlay-visual-redesign, ranked-recommendations, recommended-next-build, implementation-briefs/×4, open-questions); pointer added in `docs/current/README.md`.
- **Risk:** None (docs-only; EF-Map/overlay/CivilizationControl inspected read-only via throwaway scratch clones; vendor untouched).
- **Gates:** N/A (research docs). Markdown relative-link check run over the new set.
- **Follow-ups:** Operator to answer `open-questions.md`; suggested immediate step = the ~2-hour extraction spike (confirm module powergrid + Root hull base stats in dogma) which de-risks the whole MVP.

## 2026-06-27 — EVE Frontier Project Ideation Pass (research)
- **Goal:** Run an exhaustive, evidence-driven ideation pass across current `world-contracts` (v0.0.24), builder docs/scaffold, EF-Map, the EF-Map overlay/helper, CivilizationControl, the historical archive, and the web/community — and produce a ranked set of buildable new EVE Frontier project ideas.
- **Decision:** Executed as 7 parallel read-only subagent workstreams (A–G). Synthesized 26 scored idea cards + 8 research artifacts under `docs/current/research/project-ideation-2026-06-27/`. No code/contracts built (research-only, per task). **Recommended next build: "Frontier Changelog" (I-1)** — a public static-data differ over EF-Map's `blueprint_data_v5.json` (fills a verified gap; zero unverified contract assumptions; helps Cycle 6 relearning). High-upside alternative: **Rift Watch (I-2)** pending a live-emission kill test; best SSU-focus flagship: **SSU Inventory Intelligence (I-3)**.
- **Key findings:** Current contracts are read-path-dominant (no-wallet tools = max adoption). EF-Map (Cycle 6 cutover live 2026-06-25) already covers map/route/calculator/killboard/SSU-finder → build into its documented gaps (no calculator permalink; no static-data differ; no dormant-gate routing). Freshest on-chain surfaces: rifts (plaintext location broadcast), `inventory_key`/V2 SSU events, gate jump-permit events. Cycle 6 "Sanctuary" = wipe + single Root hull + cutting laser + star-heat + Crude-from-rifts fuel economy.
- **Caveats:** On-chain event emission on the live server was NOT verified (each on-chain idea has a kill test). Direct player sentiment (Discord/Reddit) was not web-reachable — revalidate. Turret nuance: ship turret weapons removed, but on-chain `turret.move` assembly persists in v0.0.24; turret projects excluded per operator.
- **Files:** new `docs/current/research/project-ideation-2026-06-27/**` (README, source-review, prior-art-and-exclusions, opportunity-map, idea-cards, ranked-shortlist, recommended-next-build, implementation-briefs/, open-questions); pointers added in `docs/current/README.md` and `docs/current/future-research-briefs/weekend-project-ideation.md`.
- **Risk:** None (docs-only; no submodule, code, or contract changes; vendor untouched).
- **Gates:** N/A (research docs). Markdown relative-link check run over the new set.
- **Follow-ups:** Operator to answer `open-questions.md` (horizon, kill tests, sentiment check) and pick a build. Suggested immediate step: run the I-1 2-hour spike + I-2 1-hour rift kill test in parallel.
...
- **Key SDK findings:** SealClient needs `serverConfigs` (not `serverObjectIds`), `tx.build({ onlyTransactionKind: true })` required for decrypt PTB, `getObject({ options: { showContent: true } })` for JSON content (not `core.getObject`).
- **Files:** `sandbox/shadow-broker-validation/` (Move contracts + TS scripts), `docs/operations/shadow-broker-validation-plan.md` (updated), `notes/sbp-validation-evidence.md` (new)
- **Diff:** ~800 LoC (Move + TypeScript validation scripts)
- **Risk:** Medium — new SDK surface, testnet-dependent (Seal key servers, Walrus aggregators)
- **Gates:** build ✅ test ✅ walrus ✅ seal ✅ e2e ✅
- **Follow-ups:** Port validated patterns to hackathon submission repo; consider latency optimization (Walrus uploads ~20s each)
