...
| A-31 | ~~NO event emitted for `authorize_extension`~~ *(Correction 2026-03-04: v0.0.15 added `ExtensionAuthorizedEvent` on Gate, SSU, and Turret. Fields: `assembly_id`, `extension_type`.)* | gate.move | Contract behavior | Medium | No | ~~Must poll gate objects~~ Subscribe to `ExtensionAuthorizedEvent` |
