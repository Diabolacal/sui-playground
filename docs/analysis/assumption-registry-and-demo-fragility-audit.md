...
| A-31 | NO event emitted for `authorize_extension` — extension changes are silent on-chain | gate.move (no event in authorize_extension) | Contract behavior | Medium | No | Must poll gate objects to detect extension state |
