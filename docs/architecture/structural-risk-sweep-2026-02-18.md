...
> **RESOLVED (v0.0.15):** `withdraw_item<Auth>` now accepts a `quantity: u32` parameter, enabling partial withdrawals. Full-stack-only constraint is lifted. TradePost can sell partial quantities natively.
...
| **I4** | `withdraw_item<Auth>` → `buy()` | Full-stack withdrawal acceptable | **~~DESIGN CONSTRAINT~~** ~~(no partial withdraw)~~ **RESOLVED (v0.0.15)** — partial withdraw now supported via `quantity: u32` param |
...
| **2** | **~~Partial-quantity withdrawal impossible~~** — ~~`withdraw_item<Auth>()` returns full Item stack; no `split_item` or `partial_withdraw` exists~~ | **~~HIGH~~** **RESOLVED** | ~~[inventory.move L269–L290](vendor/world-contracts/contracts/world/sources/primitives/inventory.move#L269-L290) — `withdraw_item` removes entire entry from VecMap.~~ | ~~TradePost listings CANNOT sell partial quantities.~~ | **RESOLVED (v0.0.15):** `withdraw_item<Auth>` now accepts `quantity: u32`, enabling partial withdrawals. Full-stack-only workarounds are no longer needed. TradePost can natively support partial-quantity listings. |
...
| A10 | ~~`withdraw_item<Auth>` returns full Item (no partial withdraw)~~ | ~~**PROVEN**~~ **SUPERSEDED** | ~~Source-confirmed: [inventory.move L269–L290](vendor/world-contracts/contracts/world/sources/primitives/inventory.move#L269)~~ | ~~Design around full-stack listings~~ |

> **RESOLVED (v0.0.15):** `withdraw_item<Auth>` now accepts `quantity: u32` for partial withdrawals. Full-stack-only assumption A10 is no longer valid.
