...
5. **The multi-entry portfolio strategy ($15–17k expected value) rests on an unconfirmed assumption.** Hackathon rules prohibit multi-team participation but are silent on multi-entry per team. One organizer message resolves this.
...
| **5** | **Multi-entry portfolio strategy unconfirmed** — rules say "no multi-team" but are silent on multi-entry per team | **MEDIUM** | [hackathon-event-rules-digest.md](docs/research/hackathon-event-rules-digest.md) — "Max 5 members per team; no multi-team participation." [hackathon-portfolio-roadmap.md](docs/strategy/hackathon-portfolio-roadmap.md) — 4+1 entry strategy with $15–17k expected value | If multi-entry is prohibited, the entire portfolio strategy (Track A–D) collapses to a single entry. This doesn't affect CivilizationControl's viability (it remains the strongest single entry) but eliminates bonus prize targeting via side entries. | **Pre-March-1:** Send one message to organizers: "Can a single team submit multiple entries under different GitHub repos?" Binary answer. If no, collapse to single CivilizationControl entry and redirect all effort to polish. |
...
| A11 | `deposit_item<Auth>` merges quantities for existing type_id | **PROVEN** | Source-confirmed: [inventory.move L225–L267](vendor/world-contracts/contracts/world/sources/primitives/inventory.move#L225) (v0.0.12) | — |
